...
A study was conducted on web searches of computer science students.  If these students did not click on one of the links that came up in the search, the researchers asked them to record the reason.  In the 908 trials conducted, there were 116 searches in which the students did not click on any links.  The reason each student gave for not clicking on any links is given in the data file [no_click_query.xlsx](./Data/no_click_query.xlsx). Someone used R to tally these responses. The summarized results are given in the file
[no_click_query_summary.xlsx](./Data/no_click_query_summary.xlsx). Notice that each row in the "summary" data file represents the responses of several individuals.  The *count* column tells how many times each response was given. We will used the summarized data for our analysis.

Read the data in the file [no_click_query_summary.xlsx](./Data/no_click_query_summary.xlsx) into R. These data are summarized in the table below.

```{r, include = FALSE}
library(readxl)
library(kableExtra)
library(tidyverse)
no_click_query_summary <- read_excel("./Data/no_click_query_summary.xlsx")
```

```{r, echo = FALSE}
no_click_query_summary %>% 
  select(-source) %>% 
  bind_rows(data.frame(reason = "Total", result = "", count = 116)) %>% 
  kable() %>% 
  kable_styling(full_width = F)
```

For example, 12 students indicated that "Abstracts were Enough". A total of 18 students stated that they "Checked for New Results". If we add up all the counts, there were 116 people who indicated a reason for not clicking on any links. 

We can compute the sample proportion for each group by taking the number of responses in each group and dividing this value by the total number of responses, 116.

```{r, echo = FALSE}
no_click_query_summary %>% 
  select(-source) %>% 
  mutate(total = sum(count)) %>% 
  mutate(prop = round(count / total * 1000)/1000) %>% 
...
Note that the data taken for this study are categorical.  Each of the students' responses is a categorization of their reason for not clicking on any of the links.  Even though we can summarize the data by counting the number of each type of response, the individual responses are categorical, not quantitative. 

We can do arithmetic on quantitative data. For example, it makes sense to talk about one person being one centimeter taller than another person. It also makes sense to say that one person is twice as tall as another person. 

Categorical data represent labels or categories, rather than quantitative responses. It does not make sense to do arithmetic on these. For example, colors are categorical. We would not say that "red" is double "blue". A phone number is a categorical variable. It is a label for the phone that rings when that number is dialed. It does not make sense to talk about doubling phone numbers or adding to phone numbers together.

Up to this point in the course, we have discussed methods that are appropriate when we have quantitative data. In this unit, we will learn how to describe categorical data and make inferences from them. 

As a caution, every problem in this course will deal with numbers in some way. It is not sufficient to use the presence of numbers to determine if a situation involves quantitative or categorical data. To determine whether data are quantitative or categorical, think about one subject (participant) in the study. What was measured on that one person? Is it something quantitative, like their height, income, or age? Or, is it something categorical, like a reason for not clicking on a link or whether they are an adult (yes or no)?
...
The reasons students did not click in their searches is summarized using a pie chart. Notice that "Irrelevant Results" corresponds to the largest slice. This is the most common response given by the students.
...
Read in the data: [no_click_query_summary.xlsx](./Data/no_click_query_summary.xlsx). (Review of [reading in data](RHelp.html#reading-in-the-data).)

Then, use the `barplot(...)` function to create a bar chart of the values.

<div style = "padding-left:40px;">

```{r, label = "Something is wrong here...", eval=FALSE, comment = NA}
#Create a plot of the counts in mytable:
library(RColorBrewer)
barplot(height = no_click_query_summary$count, legend.text = no_click_query_summary$reason, col = brewer.pal(n = 12, name = "Set3"))
barplot(count ~ reason, data = no_click_query_summary, col = brewer.pal(n = 12, name = "Set3"))
barplot(no_click_query_summary$reason, horiz = TRUE)
```
...
```{r, label = "Something is wrong here too...", eval=FALSE, comment = NA}

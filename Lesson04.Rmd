...
Caution: if the data you are using has `NA` or missing values in it (like the "wrong-patient" column of the WrongSiteWrongPatient data) then you need to use the `na.rm=TRUE` option to remove them, like this:
...
```{r, comment=NA}
mean(WrongSiteWrongPatient$`Wrong Patient`, na.rm=TRUE)
median(WrongSiteWrongPatient$`Wrong Patient`, na.rm=TRUE)
sd(WrongSiteWrongPatient$`Wrong Patient`, na.rm=TRUE)
...
```{r, comment=NA}
summary(WrongSiteWrongPatient$`Wrong Patient`)
```

In this case, there were 235 "NA's" or missing values in the `Wrong Patient` data.
...
- Use the option `na.rm=TRUE` to remove `NA` or missing values from the data when using any of the functions `mean(..., na.rm=TRUE)`, `median(..., na.rm=TRUE)`, `sd(..., na.rm=TRUE)`, `quantile(..., na.rm=TRUE)`.

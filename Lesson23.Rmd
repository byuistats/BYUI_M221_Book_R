...
Download these data from https://byuistats.github.io/M221R/data/estuarine_crocodiles.xlsx.
...
Consider the following scatterplot of these data, which includes a superimposed regression line.
...
Notice how the data have a linear pattern.  This data set shows a very strong linear relationship.  In many cases, the data will show more spread than is illustrated here.
...
If the histogram of the residuals shows a mound shape that is roughly symmetrical, we can conclude the assumption of normal error terms is not violated. For the estuarine crocodile data, it is reasonable to conclude the assumption of normally distributed error terms is satisfied.
...
Here is an excerpt of the `manatees` data set:  

```{r, comment = NA, echo = FALSE}
pander::pander(head(manatees[,c(2,3)]))
```

...
## Case Study: Web Searches

A study was conducted on web searches of computer science students.  If these students did not click on one of the links that came up in the search, the researchers asked them to record the reason.  There were 908 trials conducted and there were 116 searches in which the students did not click on any links.  The reasons the students gave for not clicking on any links are summarized in the data file [NoClickQuery.xlsx](./Data/NoClickQuery.xlsx).
...
### R Instructions for Describing Categorical Data
...
* Use the table function to count up how many times certain observations occur in data.
...
* Use the `barplot(...)` function and either the `cbind(...)` function or the `table(...)` function to compute the height of each bar.
* Specify colors using either `col="skyblue"` to give all bars the same color, or use `col=c("orange","skyblue","firebrick")` to give each bar a different color. Of course, use any color names you like.

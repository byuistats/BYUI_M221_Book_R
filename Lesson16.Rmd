...
* Determine the mean, standard deviation, and shape of the sampling distribution of the sample proportion given the population proportion and a specific sample size
* Calculate probabilities using a distribution of normally distributed sample proportions
...
Grades of students in a general education course at BYU-Idaho were recorded. These values are given in the data frame `grades_gender`. Download these data from https://byuistats.github.io/M221R/data/grades_gender.xlsx.
...
```{r, include = FALSE}
library(openxlsx)
grades_gender <- read.xlsx("https://byuistats.github.io/M221R/data/grades_gender.xlsx")
```

We can use R to tally (count) these responses. The counts of the number of respondents who claimed each religion is given in the file
`grades_tally`. 
Download these data from https://byuistats.github.io/M221R/data/grades_tally.xlsx.
You can [click here](https://byuistats.github.io/M221R/RHelp.html#Reading_in_Data) for instructions on how to import the dataset into R.

```{r, include = FALSE}
library(openxlsx)
grades_tally <- read.xlsx("https://byuistats.github.io/M221R/data/grades_tally.xlsx")
```

Notice that each row in the "tally" data file represents responses from several individuals.  The *count* column tells how many times each response was given. We will used this summarized data for our analysis.

The `grades_tally` data are summarized in the table below.
...
Load the data frame `grades_tally` into R. Download these data from https://byuistats.github.io/M221R/data/grades_tally.xlsx.
You can [click here](https://byuistats.github.io/M221R/RHelp.html#Reading_in_Data) for instructions on how to import the dataset into R.
...
2. The data frame `coin_heads` contains data representing a collection of 900 students' results, where each tossed a coin $n=25$ times and calculated the proportion of heads. Download these data from https://byuistats.github.io/M221R/data/coin_heads.xlsx.
You can [click here](https://byuistats.github.io/M221R/RHelp.html#Reading_in_Data) for instructions on how to import the dataset into R.

```{r, include = FALSE}
library(openxlsx)
coin_heads <- read.xlsx("https://byuistats.github.io/M221R/data/coin_heads.xlsx")
```

The following histogram illustrates these data. How would you describe the shape of the distribution of the observed proportions?

```{r, echo = FALSE}
hist(coin_heads$proportion, prob=TRUE, breaks = seq(0.1,0.9,0.04),#density=20, breaks=20,  
     xlab = "Proportion of Heads in 25 Coin Tosses", #ylim=c(0, 2), 
     ylab = "",
     main = "",
     yaxt="n",
     col = "steelblue3")
curve(dnorm(x, mean=mean(coin_heads$proportion), sd=sd(coin_heads$proportion)), 
      col="tan3", lwd=2, add=TRUE, yaxt="n")
```

<a href="javascript:showhide('Q2')"><span style="font-size:8pt;">Show/Hide Solution</span></a>
<div id="Q2" style="display:none;">
* The histogram shows that the observed sample proportions are normally distributed.
</div>
...
The proportion of heads tossed by the first student in the data frame `coin_heads` is $\widehat p = 0.68$.
...
<div class="QuestionsHeading">Answer the following questions:</div>
<div class="Questions">
11. The second student with data listed in the data frame `coin_heads` had $\widehat p = 0.44$, or 44% of their coin tosses resulted in heads.  Find the $z$-score corresponding to this value.

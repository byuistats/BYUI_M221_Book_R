...
First, read in the data from the file [chiropractic_care_table.xlsx](./data/chiropractic_care_table.xlsx).

```{r, include = FALSE}
library(readxl)
chiropractic_care_table <- read_excel("./data/chiropractic_care_table.xlsx")
```
...
After reading in the table
```{r, comment = NA}
contingency_table <- as.matrix(data.frame(chiropractic_care_table, row.names = 1))
```
...
It is usually helpful to look at the data after reversing the roles of the rows and the columns. We can transpose the data (swap the roles of the rows and columns in the data frame) using the command `t(...)`.

```{r}
barplot(t(contingency_table), beside = TRUE, legend = TRUE)
```

You will probably prefer one of these plots over the other. Either plot can be used. They show the same data in different ways.

Notice that the legend is on top of the bars for the United States in these plots. If we increase the length of the y-axis to 325 (instead of the default value), the legend no longer covers the bars. We can also color the plot by providing enough colors for each of the legend entries. We will use the first plot, which has five legend entries. So, we will provide five colors: `col = c("brown3","darkolivegreen4","tan3","darkorchid","steelblue3")`.
...
The predominant motivation that people choose to seek chiropractic care in any region is for *self care*. Compared to the other regions, people in Europe, are less likely to be motivated by the *sick role*. People in Austrailia are least likely to seek chiropractic care for *prevention*.
...
```{r, comment = NA}
#chiropractic_care_table is a table of counts created previously.

chisq_test_results <- chisq.test(contingency_table)
chisq_test_results
```

...
chiropractic_care_table %>% 
  ...
  uncount(value) %>% 
  rename(motivation = Location, location = name) %>% 
  mutate(rand = runif(n())) %>% 
...
The data from the study are given in the file [chiropractic_care_raw.xlsx](./data/chiropractic_care_raw.xlsx).
Each row represents the responses of one person. There were a total of 1316 participants in this study.
...
chiropractic_care_raw <- read_excel("./data/chiropractic_care_raw.xlsx")
...
<!--
+-------------+--------+-----------+---------------+-----------+
| Location    | Europe | Australia | United States | **Total** |
+=============+:======:+:=========:+:=============:+:=========:+
| Wellness    | 62     | 84        | 85            | **184**   |
+-------------+--------+-----------+---------------+-----------+
| Prevention  | 61     | 70        | 97            | **163**   |
+-------------+--------+-----------+---------------+-----------+
| At Risk     | 72     | 75        | 82            | **207**   |
+-------------+--------+-----------+---------------+-----------+
| Sick Role   | 64     | 82        | 111           | **227**   |
+-------------+--------+-----------+---------------+-----------+
| Self Care   | 142    | 170       | 178           | **535**   |
+-------------+--------+-----------+---------------+-----------+
| **Total**   | **282**| **469**   | **565**       | **1316**  |
+-------------+--------+-----------+---------------+-----------+
|             |        |           |               |           |
+-------------+--------+-----------+---------------+-----------+

These data are available in the file [chiropractic_care_table.xlsx](./data/chiropractic_care_table.xlsx). Notice that in this data file, there are no values giving the sum of the data in each row or column. Before we perform a chi-square analysis, it is important that we remove the row and column totals, if they are given.
-->
...
First, read in the data from the file [chiropractic_care_raw.xlsx](./data/chiropractic_care_raw.xlsx).
...
```{r, comment = NA}
head(chiropractic_care_raw.xlsx)
```

After reading in the data, we can organize it into a contingency table.
```{r}
...
Notice that the values in the first column of the contingency table are in the first row of its transpose. The rows and columns essentially swap roles.

We can use the table of observed counts to create a clustered bar chart using the `barplot( )` command. We include the following optional commands:
...
It is usually helpful to look at a bar chart of the transpose of the data.

```{r}
barplot(t(contingency_table), beside = TRUE, legend = TRUE)
```

Either plot can be used. They show the same data in different ways. You will probably prefer one of these plots over the other, so use the one you feel best describes the data. 

Notice that the legend is on top of the bars for the United States in these plots. If we increase the length of the y-axis to 325 (instead of the default value), the legend no longer covers the bars. We can also color the plot by providing enough colors for each of the legend entries. We will use the first plot, which has five legend entries. So, we will provide five colors: `col = c("brown3", "tan3", "darkorchid", "steelblue3", "darkolivegreen4")`.

```{r}
barplot(t(contingency_table), beside = TRUE, legend = TRUE,
        ylim = c(0, 325),                              # extend the y-axis to 325
        col = c("brown3", "tan3", "darkorchid", "steelblue3", "darkolivegreen4"))
```
...
The predominant motivation that people choose to seek chiropractic care in any region is for *self care*. The second-most common reason differs across the three regions. For Australia, it is *Wellness*. For Europe, it is *At Risk*, and for the United States, it is *Sick Role*. 

<br/>

<img src = "./Images/Step4.png">

Before drawing conclusions about the data depicted in a bar chart it is important to conduct the chi-squared test of independence. The $P$-value from this test will help us know if there are statistically significant differences in the ratios across the groups. 
If we fail to reject the null hypothesis, then *motivation* and *location* are independent (i.e., not related). So any patterns we are seeing in the bar plot can be attributed to random chance. However, if we reject the null hypothesis then we can claim that *motivation* and *location* are **not independent** (meaning the proportions of the reasons people seek care is different in the three locations). We can then draw our final conclusions regarding the patterns in the side-by-side bar plots.
...
The "X-squared" value of 49.743 is the test statistic of the test. Technically, it should be typeset as $\chi^2 = 49.743$. The text "X-squared" is used to represent the Greek character chi-squared ($\chi^2$) in the chi-square test output. The degrees of freedom (**df**) and the $P$-value are also given in this output.
...
2. Each expected count must be greater than or equal to 5.
    ...
    * To check expected counts in R, use the code:
    ...
    ```{r, comment = NA}
    chisq_test_results$expected
    ```

    * As shown in the above table, the smallest expected count is `63.71289`, which is well above the required 5.
...
This test is used to compare the observed counts of two or more categories to the expected counts for those same categories. For example, the Mars company claims that the colors of M&Ms are distributed as follows: 24% Blue, 13% Brown, 16% Green, 20% tan3, 13% Red, 14% Yellow. (see http://joshmadison.com/2007/12/02/mms-color-distribution-analysis/) These are the expected proportions (not counts). In a bag of $200$ M&Ms, the expected number of Blue M&Ms is $48$, the expected number of Brown M&Ms is $26$, etc.
...
$$
\begin{array}{1cl}
H_0: & \text{SIDS deaths are equally likely in winter, spring, summer, and fall.} \\
H_a: & \text{SIDS deaths are not equally likely in winter, spring, summer, and fall}
\end{array}
$$

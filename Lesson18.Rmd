...
These data are available in the file `ptc_tasting`. Download these data from https://byuistats.github.io/M221R/data/ptc_tasting.xlsx.
...
```{r, include = FALSE}
library(openxlsx)
ptc_tasting <- read.xlsx("https://byuistats.github.io/M221R/data/ptc_tasting.xlsx")
```

Note the way the data are organized in the file.  One column gives the gender, another column indicates if the individual can taste PTC, and a third column gives counts for each group.
...
<a href = "javascript:showhide('handcalculations3')"><span style = "color:white;background-color:darkgray;border-radius:5px;padding:2px;"> Show Calculations (Optional) </span></a>
<div id = "handcalculations3" style = "display:none;">

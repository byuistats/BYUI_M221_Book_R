...
* Calculate the z-score for an individual observation, given the mean and standard deviation
* Interpret a z-score
* Calculate probability as an area from a normal distribution
...
```{r, include = FALSE}
library(openxlsx)
batting_averages <- read.xlsx("https://byuistats.github.io/M221R/data/batting_averages.xlsx")
```
...
```{r}
hist(batting_averages$batting_avg,
     main = "Baseball Batting Averages", 
...
Suppose we want to estimate the probability that a randomly selected player will have a batting average that is greater than 0.280. One way to do this would be to find the proportion of players in the data set who have a batting average above 0.280. We can do this by finding the number of players whose batting averages fall into each of the blue-colored bins below and dividing this number by the total number of players.
...
hist(batting_averages$batting_avg,
     ...
     ylab = "Number of Players",
     breaks = 12,
     col = c("tan3","tan3","tan3","tan3","tan3","tan3","tan3","steelblue3","steelblue3","steelblue3","steelblue3","steelblue3"))
```

<!--```{r} -->
<!--x <- rnorm(1000) -->
<!--h <- hist(batting_averages$batting_avg, -->
<!--  breaks = 12, -->
<!--    plot = FALSE) -->
<!-- cuts <- cut(h$breaks, c(-Inf,-0.1,0,0.28,Inf)) -->
<!-- plot(h,  -->
<!--      main = "Baseball Batting Averages",  -->
<!--      xlab = "Batting Average", -->
<!--      ylab = "Number of Players", -->
...
Using this applet we can calculate proportions and probabilities based on the area under the normal curve. For the following examples, please open the [Normal Probability Applet](https://byuimath.com/apps/normprob.html) and practice using it to find areas under the curve.
...
[Lesson 3](Lesson03).) We can find the values of percentiles for a normal distribution by using the applet to calculate the $z$-score corresponding to a specified area and then plugging that $z$-score into our $z$-score equation and solving for $x$. The following instructions show how to find the 90<sup>th</sup> percentile of the distribution of batting averages.


First, we need to find the $z$-score that corresponds to the 90<sup>th</sup> percentile using the [Normal Probability Applet](https://byuimath.com/apps/normprob.html?area = 0.9&left = 1&right = 0&mid = 1).
...
<div class = "note">
For a refresher of how to make a Histogram in R, revisit [Lesson 3: R Instructions for Histograms](./docs/Lesson03.html#r-instructions-for-histograms).
</div>
...
-->
Create a histogram in R by first [importing the data](RHelp#reading-in-data) to RStudio. Then, use code similar to:
...
```{r}
hist(BodyTemp$BodyTemp, main = "Body Temperatures", xlab = "Degrees Fahrenheit", col = "steelblue3")
```
...
```{r}
mean(BodyTemp$BodyTemp) #sample mean
sd(BodyTemp$BodyTemp) #sample standard deviation
nrow(BodyTemp) #sample size
```
...
First, read in the **Survey Data** into RStudio. Then, use the code

```{r}
SurveyData <- read_excel("./data/SurveyData.xlsx")
```

```{r}
hist(SurveyData$Wage, xlab = "Hourly Wages", main = "BYU-Idaho Student Survey Results", col = "tan3")
```
...
| :------------------: | :--------------: | :--------------: |
| [Lesson 4: Describing Quantitative Data  (Spread)](Lesson04.html) | Lesson 05: <br> Normal Distributions | [Lesson 6: <br> Sampling Distribution of the Sample Mean & The Central Limit Theorem](Lesson06.html) |
</center>

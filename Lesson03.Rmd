...
<div class="SoftwareHeading">R Instructions for Histograms</div>
...
If your data set is small, then you can load the data into RStudio by using the "combine function" `c(...)`, the "assignment operator" `<-` and some name you come up with to store the data into. 
...
Now that you have created an object in R called `tuberc` that contains the 10 data points of the Tuberculosis data, you can create a histogram.

In general, you create a histogram with the code:
...
But the graph that results from this is fairly dull.
...
hist(someData, 
      col="someColorName", 
      main="Some Main Title for the Graph", 
      xlab="X-axis Label", 
      ylab="Y-axis Lable")
```
...
<div class="SoftwareHeading">R Instructions for Mean, Median, and Mode</div>
<div class="Software" style="padding:10px;">
...
2. Enter the data into a vector using the code

```{}
someNameYouCreate <- c(value1, value2, ...)
```


**Caculate a Mean**

In general, use the code
...
To calculate the median of a sample of data use the code
...
R does not directly calculate a mode, but you can tabulate (or count) how many times each value in the data occurs using the code

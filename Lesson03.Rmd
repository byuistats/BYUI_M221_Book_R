...
Here is a link to the [R Script for Lesson 3](lesson03_script.R).
...
<br>
Before we can perform calculations on a data set, we need to load the data into R. In this course, all the data can be accessed by loading the RData file below.
<br/> 
<br> 
**To load data sets for this lesson, simply run the following command in RStudio:**
```{r}
  load(url("https://byuistats.github.io/M221R/L03.RData"))
```

This will load all the data sets we need into R. You can then access them by referencing their name. For example, to view the *tuberculosis* data, simply run the command
```{r, eval = FALSE}
  View(tuberculosis)
```
...
For the tuberculosis patient costs in India, we compute the mean with the R code:
...
The value given above is the output from this command and represents the mean cost to treat the tuberculosis patients in our sample.
...
Load the data for this lesson into RStudio. 
```{r, eval = FALSE}
  load(url("https://byuistats.github.io/M221R/L03.RData"))
```

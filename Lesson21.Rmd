...
```{r, include = FALSE}
library(openxlsx)
math_self_efficacy <- read.xlsx("https://byuistats.github.io/M221R/data/math_self_efficacy.xlsx")
```
...
First, load the data frame `math_self_efficacy`. Then, run the following code to make a scatterplot:
...
plot(score ~ confidence_rating_mean, data = math_self_efficacy,
     ...
     ylab = "Exam Score of Each Student (Out of 100 Pts.)",
     main = "Study on Student Exam Performance in Math 101")
lines(c(5, 5), c(60,100), col = "tan3", lwd = 2)
points(y = 93.3, x = 5, col = "tan3", pch = 16)
points(y = 93.3, x = 5, col = "steelblue3", pch = 20)
...
plot_xy_cor <- function(df_in = df, fig_number, round_digits = 2){
  plot(y ~ x, data = df_in, 
       ...
       col = "steelblue3", 
       pch = 20, 
       main = paste("Figure",fig_number,"\nr =",round(cor(df_in$x, df_in$y), round_digits)  )
  )
}
```
...
par(mfrow = c(3,3), mar=c(1,1,3,1))
  fixed_cor(rho = -1) %>% plot_xy_cor(fig_number = 1)
  fixed_cor(rho = -0.96) %>% plot_xy_cor(fig_number = 2)
  fixed_cor(rho = -0.8) %>% plot_xy_cor(fig_number = 3)
  
  fixed_cor(rho = -0.5) %>% plot_xy_cor(fig_number = 4)
  fixed_cor(rho = 0, seed = 28) %>% plot_xy_cor(fig_number = 5)
  fixed_cor(rho = 0.5) %>% plot_xy_cor(fig_number = 6)
  
  fixed_cor(rho = 0.8) %>% plot_xy_cor(fig_number = 7)
  fixed_cor(rho = 0.96) %>% plot_xy_cor(fig_number = 8)
  fixed_cor(rho = 1) %>% plot_xy_cor(fig_number = 9)
par(mfrow = c(1,1))
```
</center>
...
par(mfrow = c(1,3), mar=c(1,1,3,1))
  ...
  fixed_cor(rho = 0.98, n = 12, seed = 4) %>% 
    bind_rows(c(x=7, y=-4)) %>% 
    plot_xy_cor(fig_number = 10)
  
  fixed_cor(rho = -0.95, n = 19, seed = 3) %>% 
    bind_rows(c(x=-5, y=-5.3)) %>% 
    plot_xy_cor(fig_number = 11)
  
  fixed_cor(rho = 0, n = 12, seed = 5) %>% 
    bind_rows(c(x=3, y=11)) %>% 
    plot_xy_cor(fig_number = 12)
...
par(mfrow = c(2,2), mar=c(1,1,3,1))
  ...
  set.seed(121)
  data.frame(x = runif(40, -1.3, 1.3)) %>% 
    mutate(y = (x-1)*x*(x+1)) %>% 
    plot_xy_cor(fig_number = 13)
  
  set.seed(120)
  data.frame(x = runif(40, -2, 2)) %>% 
    mutate(y = (x-1)*x*(x+1)) %>% 
    plot_xy_cor(fig_number = 14)
  
  set.seed(131) # 110, 111, 119
  data.frame(x = runif(50, -2, 2)) %>% 
    mutate(y = x^2 + rnorm(length(x), 0, 0.5)) %>% 
    plot_xy_cor(fig_number = 15)
  
  set.seed(29) # 23
  data.frame(x = runif(50, -1.5, 2.5)) %>% 
    mutate(y = x^2 + rnorm(length(x), 0, 0.6)) %>% 
    plot_xy_cor(fig_number = 16)
...
- Figures 14 and 16 both have relatively high correlation coefficient, $r = 0.8$ and $r = 0.75$, respectively.
</div>
<br>
...
Researchers observed 272 eruptions of this geyser <!--<cite>Azzalini90</cite>-->. The researchers recorded the duration of each eruption (in minutes) and the waiting time until the next eruption (in minutes.) The data are given in the file `old_faithful`. Download these data from https://byuistats.github.io/M221R/data/old_faithful.xlsx.
You can [click here](https://byuistats.github.io/M221R/RHelp.html#Reading_in_Data) for instructions on how to import the dataset into R.

```{r, include = FALSE}
library(openxlsx)
old_faithful <- read.xlsx("https://byuistats.github.io/M221R/data/old_faithful.xlsx")
```
...
<a href = "javascript:showhide('old_faithful_hist')"><span style = "color:white;background-color:darkgray;border-radius:5px;padding:2px;"> Show Code </span></a>
<div id = "old_faithful_hist" style = "display:none;">
...
- Since this distribution is bimodal, the mean of the waiting times is not the most useful prediction tool.
</div>
<br>
...
<a href = "javascript:showhide('old_faithful_scatter')"><span style = "color:white;background-color:darkgray;border-radius:5px;padding:2px;"> Show Code </span></a>
<div id = "old_faithful_scatter" style = "display:none;">

...
plot_xy <- function(df_in = df, fig_number, round_digits = 2){
  plot(y ~ x, data = df_in, 
       yaxt = 'n', xaxt = 'n', 
       col = "steelblue3", 
       pch = 20, 
...
par(mfrow = c(3,3), mar=c(1,1,3,1))
  fixed_cor(rho = -1) %>% plot_xy(fig_number = 1)
  fixed_cor(rho = -0.96) %>% plot_xy(fig_number = 2)
  fixed_cor(rho = -0.8) %>% plot_xy(fig_number = 3)
  
  fixed_cor(rho = -0.5) %>% plot_xy(fig_number = 4)
  fixed_cor(rho = 0, seed = 28) %>% plot_xy(fig_number = 5)
  fixed_cor(rho = 0.5) %>% plot_xy(fig_number = 6)
  
  fixed_cor(rho = 0.8) %>% plot_xy(fig_number = 7)
  fixed_cor(rho = 0.96) %>% plot_xy(fig_number = 8)
  fixed_cor(rho = 1) %>% plot_xy(fig_number = 9)
par(mfrow = c(1,1))
```
</center>

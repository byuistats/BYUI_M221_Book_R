...
A **vector** is simply a collection of numbers. To allow us to access the vector later, we assign it a name. If you copy the following code into the console area (the lower left-hand part of the RStudio screen) and press Enter, you will create a vector consisting of five numbers.
...
This is what we have done in the examples above.
...
<!-- THIS NEEDS TO BE CHANGED TO EXPLAIN THESE TWO METHODS
NOTICE THE TWO HEADERS ABOVE NEED TO HAVE EXPLANATION PUT IN BELOW THEM.
------------------
------------------
------------------
...
2. Copy and paste the following text into the lower left corner of the RStudio screen next to the `>` symbol. You will find this symbol below the word Console.
...
3. Then press Return. The first line of code stores the six numbers in a variable called `mydata`. The second line tells R to compute the average (or mean) of the numbers {1, 1, 2, 3, 5, 8}. You should see the following result:

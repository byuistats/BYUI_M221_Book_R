...
<br>
<div class = "message Tip">**Review:**  For a review of how to bar charts in R, read [Describing Categorical Data: Proportions; Sampling Distribution of a Sample Proportion](Lesson16.html). When making graphs, consider altering the labels in the graph to be more descriptive of your particular study.</div>
<br>
<br>
...
**Step 1.** Use RStudio to run the code in the file [one_proportion_function.R](./scripts/one_proportion_function.R). This will load a function that will conduct the test for one proportion.
...
<!--
* Note, by default the `one_prop_test(...)` function uses a 95% Confidence Level in creating the confidence interval that is provided in the summary output of the test. However, if you wanted to use a different confidence level, you would use `one_prop_test(..., conf.level = 0.99)` or `one_prop_test(..., conf.level = 0.90)`.
-->
...
<!--
```{r, include = FALSE, code = readLines("./scripts/one_proportion_function.R")}
# This chunk reads in the function to do a one-proportion t-test.
```
-->
...
For a hypothesis test, if the following conditions are satisfied:
...
As you can see from the output above, the requirements for conducting a hypotehsis test for one proportion are satisfied for the PTC data:
$$
\begin{array}{ll}
...
We can use a procedure that mimics the test for a single mean with $\sigma$ known from the lesson titled [Inference for One Mean: Sigma Known (Hypothesis Test)](Lesson09.html) to conduct a test for a single proportion.
...
From the output above, we note that $z = 1.2857$. and the $P$-value is 0.1986. 
...
Among the $n = 40$ observed indicators of health, $x = 19$ suggested good health. 
We enter this information into the `one_prop_test(...)` function:
...
This value is given in the output of the function `one_prop_test` above.
...
**Step 1.** Use RStudio to run the code in the file [one_proportion_function.R](./scripts/one_proportion_function.R). This will load a function that will allow you to compute the confidence interval for one proportion.
...
* The number of successes is entered using the parameter **x**
* The total number of trials is specified using the parameter **n**
* By default the `one_prop_test(...)` function uses a 95% confidence level. If you wanted to use a different confidence level, you can specify your level of significance using the argument `conf.level = ` as follows:
`one_prop_test(..., conf.level = 0.99)` or `one_prop_test(..., conf.level = 0.90)`.
...
```{r}
one_prop_test(x = 89, n = 118, conf.level = 0.95)
```
...
```{r}
one_prop_test(x = 114, n = 2426, conf.level = 0.99)
```
...
- The test statistic for a hypothesis test of one proportion is $z$. You can use the function `one_prop_test()` to perform the hypothesis test in R.
...
- A confidence interval for the true proportion $p$ can be computed in R using the function `one_prop_test()`.

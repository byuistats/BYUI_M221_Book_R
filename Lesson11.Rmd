...
We will use the body temperature data, [body_temp.xlsx](./data/body_temp.xlsx), collected by Dr. Mackowiak and his colleagues to conduct the test. <!-- <cite>Mackowiak92</cite> -->
...
```{r, include = FALSE}
library(readxl)
body_temp <- read_excel("./data/body_temp.xlsx")
```
...
```{r, eval = FALSE}
mean(body_temp$temperature)   # sample mean (x-bar)
sd(body_temp$temperature)     # sample standard deviation (s)
length(body_temp$temperature) # sample size (n)
```
...
For this t test, we are using the [body_temp.xlsx](./data/body_temp.xlsx) data.
...
The birth weights of all children born on December 18, 1997 at the Mater Mothers' Hospital in Brisbane, Australia were recorded<!--<cite>JSEdunn99</cite>-->.  The time of birth (on a 24 hour clock), gender, and birth weight of each child are given in the file [baby_boom.xlsx](./data/baby_boom.xlsx).
...
Once the [baby_boom.xlsx](./data/baby_boom.xlsx) dataset is [read into RStudio](RHelp.html#reading-in-data), the relevant summary statistics for the "Weight" column are computed by:

```{r, include = FALSE}
baby_boom <- read_excel("./data/baby_boom.xlsx")
```

```{r, eval = FALSE}
mean(baby_boom$weight)   # sample mean of the weights (x-bar)
sd(baby_boom$weight)     # sample standard deviation (s)
length(baby_boom$weight) # sample size (n)
```
...
<img src = "./Images/baby_boom-JSE-TwoSidedT1-2.png">
...
```{r, include = FALSE}
bleu <- read_excel("./data/bleu.xlsx")
```

```{r, eval = FALSE}
mean(bleu$score)   # sample mean (x-bar)
sd(bleu$score)     # sample standard deviation (s)
length(bleu$score) # sample size (n)
```
...
- Read in the [bleu.xlsx](./data/bleu.xlsx) data if you haven't already.
...
Answer the following questions using the data set [bleu.xlsx](./data/bleu.xlsx), which gives the BLEU scores for $n = 100$ translations from Spanish to English by the computer program Pharaoh.
...
The coins were borrowed from a bank in Belgium, one batch at a time. The weights (in grams) of the coins are given in the file [euro.xlsx](./data/euro.xlsx).
...
Read in the [euro.xlsx](./data/euro.xlsx) data. Then, compute the summary statistics. 

```{r, include = FALSE}
euro <- read_excel("./data/euro.xlsx")
```

```{r, comment = NA}
mean(euro$weight)   # sample mean (x-bar)
sd(euro$weight)     # sample standard deviation (s)
length(euro$weight) # sample size (n)
```
...
```{r, fig.height = 3}
boxplot(euro$weight, xlab = "Weight in grams", main = "Coins from a Bank in Belgium", col = "brown3", horizontal = TRUE)
```
...
```{r, comment = NA}
t.test(euro$weight, mu = 0, conf.level = 0.99)
```
...
- You will use R to carry out hypothesis testing and create confidence intervals involving $t$-distributions by using the `t.test(...)` function which follows the general format of `t.test(dataSetName$columnName, mu = , alternative = , conf.level = )` where `mu = ` the value from the null hypothesis, `alternative = ` either "two.sided", "less", or "greater", and `conf.level = ` 0.95, 0.99, or 0.90.

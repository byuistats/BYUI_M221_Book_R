...
Load this data frame into R by running the following command in R:
```{r}
...
Once the `baby_boom` dataset is loaded into RStudio, the relevant summary statistics for the "Weight" column are computed by:
...
the data frame `bleu`. You can load this data frame by running the command 
`load(url("https://byuistats.github.io/M221R/data/L11.RData"))`
in R.
...
The coins were borrowed from a bank in Belgium, one batch at a time. The weights (in grams) of the coins are given in the data frame `euro`. This data frame is loaded by running the command 
`load(url("https://byuistats.github.io/M221R/data/L11.RData"))`
in R.

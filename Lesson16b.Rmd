...
<!-- [student_grades.xlsx](./Data/student_grades.xlsx) -->

A study was conducted by the Pew Research Center on the religious affiliation of residents of the United States. A total of 1811 people were surveyed. Among other things, each person was asked to report their gender, religious affiliation, and how often they attend. These values are given in the data file [religion_survey.xlsx](./Data/religion_survey.xlsx). R can be used to tally (count) these responses. The counts of the number of respondents who claimed each religion is given in the file
[religion_tally.xlsx](./Data/religion_tally.xlsx). Notice that each row in the "tally" data file represents responses from several individuals.  The *count* column tells how many times each response was given. We will used this summarized data for our analysis.

Read the data in the file [religion_tally.xlsx](./Data/religion_tally.xlsx) into R. These data are summarized in the table below.
...
religion_tally %>% 
  select(-source) %>% 
  bind_rows(data.frame(religion = "Total", religion_code = " ", count = 1811)) %>% 
  kable() %>% 
  kable_styling(full_width = F)
```

For example, of the 1811 respondents, 51 reported belonging to The Church of Jesus Christ of Latter-day Saints.

We can compute the sample proportion for each group by taking the number of responses in each group and dividing this value by the total number of responses, 1811. The proportion of people in the sample who are members of The Church of Jesus Christ of Latter-day Saints is
$$ \frac{51}{1811} = 0.028$$
...
Categorical data represent labels or categories, rather than quantitative responses. It does not make sense to do arithmetic on these. For example, colors and phone numbers are categorical. We would not say that "red" is double "blue". A phone number is a categorical variable. It is a label for the phone that rings when that number is dialed. It does not make sense to talk about doubling phone numbers or adding to phone numbers together.

Note that the data reported from the Pew Research Center's study are categorical.  Each of the individuals' responses is a categorization of their religious affiliation.  Even though we can summarize the data by counting the number of each type of response, the religions reported by each individual are categorical, not quantitative.

As a caution, every problem in this course will deal with numbers in some way. It is not sufficient to use the presence of numbers to determine if a situation involves quantitative or categorical data. To determine whether data are quantitative or categorical, think about one subject (participant) in the study. What was measured on that one person? Is it something quantitative, like their height, income, or age? Or, is it something categorical like a religion, their favorite color, or whether they are an adult (yes or no)?
...
pie_tally <- religion_tally %>% 
  mutate(
    religion_code = if_else(count > 50, religion_code, "Other")
  ) %>% 
  group_by(religion_code) %>% 
  summarize(count = sum(count)) %>% 
  ungroup %>% 
  mutate(percent = round(count/sum(count)*100,1)) %>% 
  arrange(religion_code)

pie_tally2 <- pie_tally %>% 
  arrange(desc(count))

pie(
  pie_tally2$count, 
  label = pie_tally2$religion_code,
...
The religions specified by the respondents is summarized using this pie chart. Most of the respondents indicated their religion as "Protestant," which corresponds to the largest slice. 
...
Read in the data:  [religion_tally.xlsx](./Data/religion_tally.xlsx) into R. 
...
library(RColorBrewer)
barplot(height = pie_tally$count, legend.text = pie_tally$religion_code, las = 2, col = brewer.pal(n = 12, name = "Set3"), args.legend = list(x = "topleft"))
barplot(count ~ religion_code, data = pie_tally, las=2, cex.names = 0.45, xlab = "")
#barplot(pie_tally$religion_code, horiz = TRUE)
barplot(count ~ religion_code, data = pie_tally, las=2, cex.names = 0.45, xlab = "",args.legend = list(x = "topleft"))
par(mar=c(11,4,4,4))
barplot(count ~ religion_code, data = pie_tally, las=2, xlab = "")
barplot(count ~ religion_code, data = pie_tally, las=2, cex.names = 0.45, xlab = "")

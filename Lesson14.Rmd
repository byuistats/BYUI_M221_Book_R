...
* Read in the data: [Gratitude.xlsx](./Data/Gratitude.xlsx).
* Load `library(mosaic)`.
...
library(mosaic)
favstats(`Happiness Score` ~ `Treatment Group`, data=Gratitude)
```
...
```{r}
boxplot(`Happiness Score` ~ `Treatment Group`, data=Gratitude,
...
* Read in the data if you haven't already: [Gratitude.xlsx](./Data/Gratitude.xlsx).
...
```{r, comment=NA}
gratitude_aov <- aov(`Happiness Score` ~ `Treatment Group`, data=Gratitude)
...
```{r, eval=FALSE}
library(mosaic)
favstats(Accuracy ~ Footwear, data=SoccerShoes)
```
...
```{r}
boxplot(Accuracy ~ Footwear, data=SoccerShoes, 
        col="yellow2", 
...
```{r}
soccer_aov <- aov(Accuracy ~ Footwear, data=SoccerShoes)
summary(soccer_aov)
```

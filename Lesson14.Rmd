...
* Use the `favstats(...)` function with the quantitative data column listed first, followed by a `~`, followed by the categorical data column. Use the back tick mark `` ` `` to surround column names that have spaces in them, like `` happiness `` and `` treatment ``. 

---
title: "Lesson 14: Inference using ANOVA (Several Independent Samples)"
output: 
  html_document:
    theme: cerulean
    toc: true
...
* Perform a hypothesis test for ANOVA (several independent samples) using the following steps:
...
<td><p><img src = "./Images/gordon_b_hinckley_small.png"></p></td>
<td><p>$~ ~$</p></td>
...
### R Instructions for Numerical Summaries of Several Groups (Long Data)
...
* Load the data: `gratitude`. Download these data from https://byuistats.github.io/M221R/data/gratitude.xlsx.
...
```{r, include = FALSE}
library(openxlsx)
gratitude <- read.xlsx("https://byuistats.github.io/M221R/data/gratitude.xlsx")
```

* Load `library(mosaic)`. 
* Use the `favstats(...)` function with the quantitative data column name listed first, followed by a tilde, `~`, which is followed by the categorical data column name. We indicate that these columns are part of the `gratitude` data frame using the command `data = gratitude`.
...
### R Instructions for Side-by-Side Boxplots of Several Groups (Long Data)
...
Based on the numerical summaries and boxplot, the gratitude group had a higher life happiness mean score of 5.05 compared to the other two groups of 4.66 (Events group) and 4.67 (Hassles group). This suggests that perhaps exercising a daily recognition of things one is grateful for can result in greater life satisfaction. However, it is important to first **make inference** before we decide if the data really warrants this conclusion.
...
### R Instructions for Conducting an ANOVA Test (Long Data)
...
* Load the `gratitude` data if you haven't already,
* Use the `aov(...)` function and the assignment operator `<-` to perform and save the results of the test to any name you want to create for the test. Here we use the name `aov_output`. 
* When using the `aov(...)` function, place the quantitative variable first, followed by a `~`, followed by the categorical variable. As in other examples, we indicate the data frame using the `data = ` argument.
* Use the `summary(...)` function to view the results of the ANOVA test in a nice format.
...
There are two requirements of ANOVA that we can verify directly:

<!-- 
- The data represent a simple random sample.

    We cannot check this requirement by looking at the data. As long as the data are representative of the population, the intent of this requirement is satisfied. 
-->
...
- The variances are equal for each group.
    ...
    This requirement is checked by examining the variances. Remember, the variance is the square of the standard deviation. The rule we will use is: *if the largest variance is more than four times as big as the smallest variance, we will conclude that the variances are equal.* 

    We can find the variance using the following R command:
```{r}
var(happiness ~ treatment, data = gratitude)
```

    For the gratitude study, the variances of the Events, Grateful, and Hassles groups were 0.720, 0.892, and 0.692, respectively. The largest of these variances is 0.892 and the smallest is 0.692. Notice that the largest variance is not more than four times as big as the smallest variance. So, we conclude the assumption is satisfied.
...
## ANOVA with Wide Data: Soccer Shoes
...
Data representing the results of this study are given in the data frame `soccer_shoes`. Download these data from https://byuistats.github.io/M221R/data/soccer_shoes.xlsx.
You can [click here](https://byuistats.github.io/M221R/RHelp.html#Reading_in_Data) for instructions on how to import the dataset into R.

```{r, include = FALSE}
library(openxlsx)
soccer_shoes <- read.xlsx("https://byuistats.github.io/M221R/data/soccer_shoes.xlsx")
```


### R Instructions for Converting "Wide" Data to "Long" Format
...
soccer_shoes %>% 
    head(8) %>%  
    kable(align = c("c", "c", "c", "c", "c", "c", "l")) %>% 
    kable_styling(full_width = F)
```

There are six columns of data, where each column gives the accuracy rating for a different type of footwear. Note that this is wide data, since every column gives the data for one group. The last column provides the reference for the articles from which the data were derived.

In order to conduct an ANOVA analysis to determine if the mean accuracy is the same for the six types of footwear, we need to convert the data into "long" format.  We will do this in three steps:

  1. Examine the data. We can either do this with either the `View(soccer_shoes)` or `head(soccer_shoes)` commands.
  
  2. We will "stack" the data to create a new data frame in long format. We filter out any columns that are not numeric in this process. The column titled `comments` is deleted before the data are stacked. We give this new data frame a separate name: `soccer`. Notice that `soccer` that is in "long" format, where `soccer_shoes` is in "wide" format.
```{r}
  soccer <- stack(Filter(is.numeric, soccer_shoes))
```

Use `View(soccer)` to explore these data. Notice that R has transformed the data, so that it is now in "long" format, where the accuracy values are in a column called `values` and the label indicating ("ind") the type of footwear is in a second column called `ind`.

After making this change, the techniques for performing ANOVA on long data can be applied to the data set `soccer`.
...
Use the soccer shoes to answer the following questions.
...
Using the code above, we can pull out the important values.
  ...
  ind     N    Mean    Std. Deviation
  ------- ---- ------- ----------------
  shoe_a  20   33.10   5.230
  shoe_b  20   32.70   5.430
  shoe_c  20   29.45   5.216
  shoe_d  20   32.00   7.291
  shoe_e  20   32.20   5.569
...
* Based on the boxplot, it appears that "socks" seem to give the greatest accuracy. This is substantiated by looking at the sample means in the numerical summary where "socks" has a mean accuracy of 35.25, which is bigger than any other mean.
</div>
&nbsp;
</div>
<br>
...
* We compute the variance of the groups using the following command:
```{r}
var(values ~ ind, data=soccer)
```

The largest variance (53.158) is not more than four times the smallest (27.208), so the assumption of equal variances is satisfied.
</div>
<br>
...
* Since the $P$-value $= 0.0809 < 0.1 = \alpha$, we reject the null hypothesis.
</div>
&nbsp;
</div>
<br>
...
- We can conduct ANOVA testing when the following requirements are satisfied:
    ...
    3. The variance is constant.
        - This is satisfied when the largest variance is not more than four times the smallest.
        
- To perform an ANOVA in R, use the code `aov_output <- aov(...)` to run the test, then the code `summary(aov_output)` to view the results.

- If data are provided in "wide" format, they must first be converted to "long" format before performing the ANOVA analysis.
This can be accomplished using the command `new_data_frame <- stack(Filter(is.numeric, old_data_frame))` in R.

...
Data representing the results of this study are given in the file [soccer_shoes](./data/soccer_shoes.xlsx). (Review of [reading in data](RHelp.html#reading-in-the-data).)
...
This command removes the column `comments` from the data set. The only way to restore the column is to start over by reading in the data again.
  ...
  3. We will use the `stack()` command to create a new data set called `soccer` that is in "long" format:
```{r}

...
* First, read the data into RStudio: [reading.xlsx](./data/reading.xlsx). (Review of [reading in data](RHelp.html#reading-in-the-data).)

```{r, include=FALSE}
library(readxl)
reading <- read_excel("./data/reading.xlsx")
```
...
Read the data into RStudio: [reading.xlsx](./data/reading.xlsx).
...
* Import the data into RStudio: [reading.xlsx](./data/reading.xlsx)
...
```{r, comment=NA}
t.test(reading$nights ~ reading$group, mu=0, alternative="two.sided")
```
...
* We need to find the the following: $\bar x_1$, $s_1$, $n_1$, $\bar x_2$, $s_2$, and $n_2$. So we read in the [fifa_heart_attacks.xlsx](./data/fifa_heart_attacks.xlsx) data into RStudio, ensure `library(mosaic)` is loaded, and then use the `favstats(...)` function.

```{r, include=FALSE}
fifa_heart_attacks <- read_excel("./data/fifa_heart_attacks.xlsx")
```
...
```{r}
boxplot(fifa_heart_attacks$heart_attacks ~ fifa_heart_attacks$time_period, col=c("gray", "orange"), main="2006 World Cup \n ")
```
...
In the study by Arlene Butz, et.al., on the reading practices of children, the researchers wanted to determine if there was a difference in the mean number of nights children read in the two groups (DEV and GEN)<!--<cite>Butz09</cite>-->. A survey was given to the child's caretaker (usually a parent).  This survey included questions about the child's development and behavior. The survey also asked the number of nights each week that the child participated in reading in the home. Data representative of their results are given in the file [reading.xlsx](./data/reading.xlsx).
...
- Read in the data: [reading.xlsx](./data/reading.xlsx)
...
* First, read the data into RStudio: [copd_rehab.xlsx](./data/copd_rehab.xlsx)

```{r, include = FALSE}
copd_rehab <- read_excel("./data/copd_rehab.xlsx")
```

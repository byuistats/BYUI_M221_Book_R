...
We can compare the mean income of men to the mean income of women using the procedures of this section.  We will conduct hypothesis tests and compute confidence intervals for the difference in the true population means of two groups ($\mu_\text{DEV Group} - \mu_\text{GEN Group}$).  
...
### R Instructions for Numerical Summaries of Two Groups
...
<div class="SoftwareHeading">R Instructions for Numerical Summaries of Two Groups</div>
<div class="Software">
**To compute numerical summaries for two groups at once:**

* First, read the data into RStudio: [ReadingPractices.xlsx](./Data/ReadingPractices.xlsx).

```{r, include=FALSE}
library(readxl)
ReadingPractices <- read_excel("./Data/ReadingPractices.xlsx")
```

* Second, load the `mosaic` library in RStudio that makes calculating the numerical summaries for two groups really quick.
...
<div class="note">
If you get the error message <span style="color:red;">Error in library(mosaic) : there is no package called ‘mosaic’</span> then you will need to install the `mosiac` package using the code: `install.packages("mosaic")`

You'll only ever need to "install" this package once. But you'll need to run "library(mosaic)" every time you reopen RStudio and want to use the `mosaic` package. As long as you keep RStudio open, you won't have to re-load the mosaic library.
...
```{r, comment=NA}
favstats(Nights ~ Group, data=ReadingPractices)
```

<div class="note">
If you get the error message <span style="color:red;">Error in favstats(Nights ~ Group, data = ReadingPractices) : could not find function "favstats"</span> then you will need to run the code: `library(mosaic)` first, then try the `favstats(...)` code again.
</div>
...
### R Instructions for Side-by-Side Boxplots

<div class="SoftwareHeading">R Instructions for Side-by-Side Boxplots</div>
<div class="Software">
**To visualize the distributions of the two sample simultaneously:**

Read the data into RStudio: [ReadingPractices.xlsx](./Data/ReadingPractices.xlsx).
...
```{r}
boxplot(Nights ~ Group, data=ReadingPractices, col=c("firebrick","skyblue"), main="Reading Practices of Children")
```
...
To edit the names along the x-axis of the graph "DEV" and "GEN" use the `names=c("label 1", "label 2", ...)` command where you specify one label for each box in the boxplot.

```{r}
boxplot(Nights ~ Group, data=ReadingPractices, names=c("Behavioral Problems", "No Problems"), col=c("firebrick","skyblue"), main="Reading Practices of Children")
```


To copy and paste a graph from R into say Microsoft word, do the following:
- Create the graph in R
- Click the little "Export" button at the top of the "Plots" window
- Choose either the "Copy" or "Save" options.
- Then, either paste in the plot to Word, or insert the image (if you saved it).
</div>
<br>
</div>
...
### R Instructions for an Independent Samples t Test

<div class="SoftwareHeading">R Instructions for an Independent Samples t Test</div>
<div class="Software">
**The following instructions will help you conduct a hypothesis test for two means with independent samples in R.**

* Import the data into RStudio: [ReadingPractices.xlsx](./Data/ReadingPractices.xlsx)
...
```{r, comment=NA}
t.test(Nights ~ Group, data=ReadingPractices, mu=0, alternative="two.sided")
```
...
### Case Study: World Cup Heart Attacks
...
* We need to find the the following: $\bar x_1$, $s_1$, $n_1$, $\bar x_2$, $s_2$, and $n_2$. So we read in the [WorldCupHeartAttacks.xlsx](./Data/WorldCupHeartAttacks.xlsx) data into RStudio, ensure `library(mosaic)` is loaded, and then use the `favstats(...)` function.

```{r, include=FALSE}
WorldCupHeartAttacks <- read_excel("./Data/WorldCupHeartAttacks.xlsx")
```

```{r, message=FALSE, warning=FALSE, comment=NA}
library(mosaic)
favstats(HeartAttacks ~ TimePeriod, data=WorldCupHeartAttacks)
```
...
```{r}
boxplot(HeartAttacks ~ TimePeriod, data=WorldCupHeartAttacks, col=c("gray", "orange"), main="2006 World Cup \n ")
```
...
```{r, comment=NA}
t.test(HeartAttacks ~ TimePeriod, data=WorldCupHeartAttacks)
```
...
#### Statistics Theory: Independent Samples t Test

In this course, we do not go very deep into statistical theory.  For those students who are interested, there is a lot of theory undergirding statistical practice that you will be able to study in other statistics courses here at BYU-Idaho.
...
```{r}
boxplot(Nights ~ Group, data=ReadingPractices,
        main="Reading Practices of Children",
        col=c("firebrick","skyblue"))
```
...
### R Instructions for a Confidence Interval of the Difference of Two Means
...
<div class="SoftwareHeading">R Instructions for a Confidence Interval of the Difference of two Means</div>
<div class="Software">
**To compute a 95% confidence interval for the difference of two means from two independent samples:**

- Read in the data: [ReadingPractices.xlsx](./Data/ReadingPractices.xlsx)
- Perform an independent samples t test using the `t.test(..., conf.level=0.95)` function.

```{r, comment=NA}
t.test(Nights ~ Group, data=ReadingPractices, conf.level=0.95)
```
...
```{r, comment=NA}
ReadingPractices$GroupReversed <- factor(ReadingPractices$Group, levels=c("GEN", "DEV"), ordered=TRUE)
t.test(Nights ~ GroupReversed, data=ReadingPractices)
```
...
### R Instructions for Describing "Wide" Data

<div class="SoftwareHeading">R Instructions for Numerical Summaries with Wide Data</div>
<div class="Software">
**When the data for each group is in a separate column** numerical summaries are calculated one column at a time.

* First, read the data into RStudio: [COPD-Rehab](./Data/COPD-Rehab.xlsx)

```{r}
COPD_Rehab <- read_excel("./Data/COPD-Rehab.xlsx")
```

* Second, load the `mosaic` library in RStudio that makes calculating the numerical summaries really quick, even for just one set of data.
...
Use the `favstats(...)` function to produce numerical summaries for each group separately.
...
<div class="note">
If you get the error message <span style="color:red;">Error in favstats(...) : could not find function "favstats"</span> then you will need to run the code: `library(mosaic)` first, then try the `favstats(...)` code again.
</div>

**Boxplots can be created by using the `,` instead of the `~`:**
...
```{r}
boxplot(COPD_Rehab$Community, COPD_Rehab$Hospital, names=c("Community", "Hospital"), main="Chronic Obstructive Pulmonary Disease", ylab="Increased Distance Patients Could Walk (meters)", col=c("steelblue","steelblue3"))
```
...
### R Instructions for t Tests with "Wide" Data

<div class="SoftwareHeading">R Instructions for t Tests with Wide Data</div>
<div class="Software">
**When the data for each group is in a separate column** the independent samples t test is performed using the comma `,` instead of the tilde `~`.

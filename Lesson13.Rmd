...
* First, read the data into RStudio: [ReadingPractices.xlsx](./Data/ReadingPractices.xlsx). (Review of [reading in data](RHelp.html#reading-in-the-data).)
...
```{r, comment=NA}
favstats(ReadingPractices$Nights ~ ReadingPractices$Group)
```
...
```{r}
boxplot(ReadingPractices$Nights ~ ReadingPractices$Group)
```
...
* the `col=...` statement provide colors, either one color for both boxes in the plot `col="skyblue"`, or colors for each box in the plot, `col=c("firebrick","skyblue")`.
...
```{r, comment=NA}
t.test(ReadingPractices$Nights ~ ReadingPractices$Group, mu=0, alternative="two.sided")
```
...
library(mosaic)
favstats(WorldCupHeartAttacks$HeartAttacks ~ WorldCupHeartAttacks$TimePeriod)
```
...
```{r}
boxplot(WorldCupHeartAttacks$HeartAttacks ~ WorldCupHeartAttacks$TimePeriod, col=c("gray", "orange"), main="2006 World Cup \n ")
```
...
```{r, comment=NA}
t.test(WorldCupHeartAttacks$HeartAttacks ~ WorldCupHeartAttacks$TimePeriod)
```
...
```{r, comment=NA}
t.test(ReadingPractices$Nights ~ ReadingPractices$Group, conf.level=0.95)
```
...
t.test(ReadingPractices$Nights ~ ReadingPractices$GroupReversed)
```
...
### R Instructions for Describing Data in Two Different Columns
...
### R Instructions for t Tests with Data in Two Different Columns

...
* Apply a confidence interval for the mean of differences (two dependent samples) using the following steps:
  ...
  + Interpret a confidence interval for the mean of differences (two dependent samples)
...
Annie Mahon and other researchers in Wayne Campbell's nutrition lab studied the weight loss of $n = 27$ middle aged women who consumed a prescribed low-calorie diet. <!--<cite>Mahon07</cite>-->  The women's weights were recorded (in kilograms) at the beginning of the study and after the nine-week diet period. 

Download these data from https://byuistats.github.io/M221R/data/weight_loss.xlsx.
...
```{r, include = FALSE}
library(openxlsx)
weight_loss <- read.xlsx("https://byuistats.github.io/M221R/data/weight_loss.xlsx")
```
...
<div class = "Software">
To compute the *differences* within each paired set of measurements in R, begin by reading in the `weight_loss` data into RStudio. 
Download these data from https://byuistats.github.io/M221R/data/weight_loss.xlsx.
You can [click here](https://byuistats.github.io/M221R/RHelp.html#Reading_in_Data) for instructions on how to import the dataset into R.
...
Notice that we use $\bar d$ to represent the mean of the difference where $d = post-pre$. We use a subscript of $d$ to indicate that the standard deviation we computed is the standard deviation of the differences.

We make a histogram and boxplot of the differences:
...
```{r}
boxplot(weight_loss$difference,
        horizontal=TRUE,
        col = "tan3",
        xlab = "Weight Change in Kilograms (Post - Pre)",
        main = "Weight Loss Study") 
...
library(tidyverse)
library(knitr)
kable(weight_loss %>% select(-comments), align = "cccc")
```
...
Recall that the differences $d$ in the duration of the patient's stay in the hospital were decided to be computed in the order, $difference = infected - not_infected$.

```{r, comment = NA}
hospital_infections$difference <- hospital_infections$infected - hospital_infections$not_infected
```

The numerical summaries are computed using the command:
...
hist(hospital_infections$difference, 
     main = "Matched Pairs Study \n Nosocomial Infections", 
     xlab = "Difference in Hospital Stay (infected - not_infected)")
```

```{r}
boxplot(hospital_infections$difference,
        horizontal=TRUE,
        main = "Matched Pairs Study \n Nosocomial Infections",
        xlab = "Difference in Hospital Stay (infected - not_infected)")
```
...
Ebba Carlsson and other researchers in Sweden investigated whether listening to stressful classical music increases a person's REE. <!--<cite>Carlsson05</cite>-->  Each subject's REE was measuring during silence and again while listening to stressful classical music.  Data representing their results are given in the data frame `ree`. Download these data from https://byuistats.github.io/M221R/data/ree.xlsx.
You can [click here](https://byuistats.github.io/M221R/RHelp.html#Reading_in_Data) for instructions on how to import the dataset into R.

```{r, include = FALSE}
library(openxlsx)
ree <- read.xlsx("https://byuistats.github.io/M221R/data/ree.xlsx")
```


Notice that this is not a pre- and post-test, but it is still a test involving paired data.  Two REE measurements were made for each subject:  (1) in silence ($silence$) and (2) while listening to stressful classical music ($stressful$).


**State the null and alternative hypotheses and the level of significance**

Since we are testing for an increase in the mean REE, we let $d = stressful - silence$. Our alternative hypothesis will be that $\mu_d > 0$.  The null and alternative hypotheses are:
$$
\begin{align}
H_0: &~~ \mu_d = 0 \\
H_a: &~~ \mu_d > 0
...
First, load the `ree` data into RStudio.

Then, compute the differences using the command:
...
The first observed difference will be 50 kJ/d.
...
19. Create a histogram and boxplot of the differences.
...
```{r, comment = NA}
hist(ree$difference, 
     main = "Effect of Classical Music on Energy", 
     xlab = "Resting Energy Expenditure (Stressful - Silence)", 
     col = "darkorchid") 
```

```{r, comment = NA}
boxplot(ree$difference,
        horizontal = TRUE,
        main = "Effect of Classical Music on Energy",
        xlab = "Resting Energy Expenditure (Stressful - Silence)",
        col = "darkorchid") 
...
Allie Henrich, a BYU-Idaho student, compared the lowest published ticket prices of one-way flights from Heathrow to various destinations in Europe.  Using Travelocity.com, she recorded the lowest published fares for nonstop midweek flights booked either 14 days in advance or 90 days in advance.  The prices (in US dollars) are given in the file `direct_flight_costs`. Download these data from https://byuistats.github.io/M221R/data/direct_flight_costs.xlsx.
You can [click here](https://byuistats.github.io/M221R/RHelp.html#Reading_in_Data) for instructions on how to import the dataset into R.

```{r, include = FALSE}
library(openxlsx)
direct_flight_costs <- read.xlsx("https://byuistats.github.io/M221R/data/direct_flight_costs.xlsx")
```
...
```{r}
direct_flight_costs$difference <- direct_flight_costs$fourteen_days - direct_flight_costs$ninety_days
hist(direct_flight_costs$difference, 
     col = "tan3",
...
```{r}
direct_flight_costs$difference2 <- direct_flight_costs$ninety_days - direct_flight_costs$fourteen_days
hist(direct_flight_costs$difference2, 
     col = "brown3",
...
```{r, comment = NA}
favstats(pine_beetle$before)
```

* The mean, given in the summary output above, was 1028.41 trees per hectare and the standard deviation was 57.03 trees per hectare.  Note that the values were rounded to two decimal places, since the data were given to one decimal place.
...
41. Create a histogram and boxplot of the differences in the density of the trees.
...
hist(pine_beetle$difference, 
     main = "Pine Beetle Outbreaks", 
     xlab = "Difference in Tree Density (After - Before)", 
     col = "darkolivegreen4")
```

```{r}
boxplot(pine_beetle$difference,
        horizontal = TRUE,
        main = "Pine Beetle Outbreaks",
        xlab = "Difference in Tree Density (After - Before)",
        col = "darkolivegreen4")
...
* Download these data from https://byuistats.github.io/M221R/data/pine_beetle.xlsx.
You can [click here](https://byuistats.github.io/M221R/RHelp.html#Reading_in_Data) for instructions on how to import the dataset into R.
...
* Use the code `t.test(pine_beetle$difference, conf.level = 0.95)` to set the desired confidence level and create the confidence interval.
...
The difference in the amount of time each person slept was computed by subtracting the amount of time the subjects slept when taking the drug minus the sleep duration with no drug.  The data are given in the file `sleep_drug`. 
Download these data from https://byuistats.github.io/M221R/data/sleep_drug.xlsx.
You can [click here](https://byuistats.github.io/M221R/RHelp.html#Reading_in_Data) for instructions on how to import the dataset into R.

```{r, include = FALSE}
library(openxlsx)
sleep_drug <- read.xlsx("https://byuistats.github.io/M221R/data/sleep_drug.xlsx")
```

After reading in these data, run the following to compute the column of differences:

```{r}
sleep_drug$difference = sleep_drug$treatment - sleep_drug$control
```

Run the command `View(sleep_drug)` to examine the data file.
...
```{r}
favstats(sleep_drug$difference)
```
...
Here is a histogram and a boxplot of the data:

```{r, fig.height = 3}
hist(sleep_drug$difference,
     col = "brown3",
...
```{r, comment = NA}
t.test(sleep_drug$difference, conf.level = 0.90)
```
...
- The R code to perform a paired samples t test requires two steps. First, the `differences` must be computed. Second, the `t.test(sleep_drug$difference, mu = 0, alternative = "two.sided")` should be used. Be sure to match the `alternative = ` statement to either `"less"`, `"greater"`, or `"two.sided"` so that it matches whatever is stated in the actual $H_a$ statement. 

- The R code to created a confidence interval for the true mean of the differences, $\mu_d$, uses `t.test(sleep_drug$difference, conf.level = 0.95)` where `conf.level = ` can be changed to either 0.9 (for a 90% confidence interval) or 0.99 (for a 99% confidence interval).

...
<div class="Software">
To compute the *differences* within each paired set of measurements in R, begin by reading in the [Mahon.xlsx](./Data/Mahon.xlsx) data into RStudio. (If you need a reminder on how to read in data, [click here](RHelp.html#reading-in-data).)

Then, calculate the *differences* using the assignment operator `<-` and the selection operator `$`:

```{r, include=FALSE}
library(readxl)
Mahon <- read_excel("./Data/Mahon.xlsx")
```

```{r}
differences <- Mahon$Post - Mahon$Pre
```
...
```{r, comment=NA}
summary(differences) #mean and five-number summary
sd(differences) #standard deviation
length(differences) #sample size
hist(differences, main="Weight Loss Study", xlab="Weight Change in Kilograms (Post - Pre)", col="wheat2") #histogram
```
...
<div class="Questions">
2. Following the directions above, compute the difference in the women’s weights yourself in R. What is the difference in the post- and pre-study weights for the first person included in the data file? 
...
```{r, echo=FALSE}
hist(differences, main="Weight Loss Study \n All 27 Women Lost Weight", xlab="Weight Change in Kilograms (Post - Pre)", col="wheat2")
```
...
The requirement of normality is satisfied for Mahon's data. Though the sample size (n=27) is not quite up to 30, the histogram of differences looks fairly normal, so $\bar d$ will be approximately normal and the requirement is satisfied. If the data had been highly skewed, then we would be worried about the results of the test and would have recommended they redo the study with a larger sample size in order to have confidence in the results of the t-test.
...
```{r, comment=NA}
t.test(differences, mu=0, alternative="less")
```
...
```{r, include=FALSE}
NosocomialInfections <- read_excel("./Data/NosocomialInfections.xlsx")
```
...
Recall that the differences $d$ in the duration of the patient's stay in the hospital were decided to be computed in the order, $differences = Infected - Notinfected$.

```{r, comment=NA}
differences <- NosocomialInfections$Infected - NosocomialInfections$NotInfected
```
...
```{r}
hist(differences, main="Matched Pairs Study \n Nosocomial Infections", xlab="Difference in Hospital Stay (Infected - Notinfected)")
```
...
The p-value can be found in the R output. The calculation below shows how the p-value in R is being calculated:
<center>
$$
...
As a reminder, scientific notation of $1.296e^{-7}$ is equivalent to $1.296 \times 10^{-7}$. The $-7$ tells us to move the decimal place to the left seven times, causing the number to change from $1.296$ to $0.0000001296$. 
...
First, read in the [REE-ClassicalMusic](./Data/REE-ClassicalMusic.xlsx) data into RStudio.

```{r, include=FALSE}
REE_ClassicalMusic <- read_excel("./Data/REE-ClassicalMusic.xlsx")
```
...
```{r, comment=NA}
hist(differences, main="Effect of Classical Music on Energy", xlab="Resting Energy Expenditure (Stressful - Silence", col="orchid") 
```
...
* Requirement: **The sample mean ($\bar{d}$) is normally distributed.**    
Because our sample size (n=40) of differences is greater than 30, we can be sure the sampling distribution of $\bar d$ will be normal.
...
* The test statistic is plotted on the horizontal axis.  The $P$-value is shaded in green. The same value can be found in the R output.  
...
```{r, include=FALSE}
DirectFlightCosts <- read_excel("./Data/DirectFlightCosts.xlsx")
```

```{r}
differences <- DirectFlightCosts$FourteenDays - DirectFlightCosts$NinetyDays
hist(differences, main="Direct Flight Costs", xlab="Difference in Dollar Cost (14 Days - 90 Days)", col="yellow3", breaks=30)
```
...
* Perform a `t.test(...)` on the differences.
* Set the desired confidence level using `t.test(..., conf.level=0.95)`.
<br>
...
```{r, include=FALSE}
PineBeetle <- read_excel("./Data/PineBeetle.xlsx")
```

```{r, comment=NA}
summary(PineBeetle$Before)
nrow(PineBeetle)
sd(PineBeetle$Before)
```
...
```{r, comment=NA}
differences <- PineBeetle$After - PineBeetle$Before
summary(differences)
length(differences)
sd(differences)
```
...
```{r}
hist(differences, main="Pine Beetle Outbreaks", xlab="Difference in Tree Density (After - Before)", col="forestgreen")
```
...
* Requirement: **The sample mean ($\bar{d}$) is normally distributed.**    
For the pine beetle data, the histogram indicates that the data are not normally distributed.  However, since the sample size is large ($n = 170$), we can know the sample mean $\bar{d}$ is normally distributed despite the skew in the data.
...
```{r}
differences <- c(0.7, -1.6, -0.2, -1.2, -0.1, 3.4, 3.7, 0.8, 0, 2)
```
...
```{r, fig.height=3}
boxplot(differences, xlab="Difference in Hours of Sleep", main="Slee Inducing Drugs Study", horizontal=TRUE, col="firebrick")
```
...
* Requirement: **A simple random sample was drawn from the population.**    
We assume the subjects represent a random sample from the population.

* Requirement: **The sample mean ($\bar{d}$) is normally distributed.**    
Because the sample size is small, with only 10 values, it is hard to see a shape in a histogram of the data, regardless of the number of bins chosen. However, it is clear that the data is not strongly skewed. We will move forward, assuming the requirement of a normal sampling distribution of \bar d is not violated.
Remember, just like the hypothesis test, this confidence interval for a mean of differences is robust. That means it is not very sensitive to the requirements. If they are violated, this interval estimate will probably still give reasonably good results.
...
```{r, comment=NA}
t.test(differences, conf.level=0.90)
```
...
- The R code to perform a paired samples t test requires two steps. First, the `differences` must be computed. Second, the `t.test(differences, mu=0, alternative="two.sided", conf.level=0.95)` should be used. Be sure to match the `alternative=` statement to either `"less"`, `"greater"`, or `"two.sided"` so that it matches whatever is stated in the actual $H_a$ statement. Also, change `conf.level=` to either 0.9 (for a 90% confidence interval) or 0.99 (for a 99% confidence interval).

...
You can access this data frame by running the command
```{r}
...
The difference in the amount of time each person slept was computed by subtracting the amount of time the subjects slept when taking the drug minus the sleep duration with no drug.  The data are given in the file `sleep_drug`. You can access this data frame by running the command
`load(url("https://byuistats.github.io/M221R/data/L12.RData"))`
in R. After reading in these data, run the following to compute the column of differences:
